...
void createAreaManagers(vector<AreaManager*> &managers, int& numOfManagers, int& numOfAreas)
{
	...
	//vector<AreaManager*>::iterator managerItr = managers.begin();
	...
	for (int i = 0; i < numOfManagers && i < numOfAreas; i++)
	{
		try
		{
			//managers.insert(managerItr,new AreaManager(*nameItr,*idItr,*salaryItr,*yearsItr));
...
void addAllAnimalsToZoo(Zoo& myZoo, vector<Animal*>& animals, int& numOfAnimals)
{
	...
	myZoo.addAnimal(AdapterLion("lio",250,2004,Lion::BROWN), (*--areasItr)->getName());
}
...
void freeAllAreas(vector<Area*>& areas)
{
	vector<Area*>::iterator itBegin = areas.begin();
	vector<Area*>::iterator itEnd = areas.end();
